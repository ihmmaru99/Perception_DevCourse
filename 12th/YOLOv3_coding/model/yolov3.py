...
class Yololayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # if kitti data, n_calsses is 8. C = (8 + 5) * 3 = 39
        '''
        filters = (classes + 5) * number of masks in one yolo layer

          = (classes + width + height + x + y + confidence) * number of masks in one yolo layer

          = (classes + 1+1+1+1+1) * 3

​

        ==> Yolov3은 grid cell마다 3개의 박스를 detect하므로 filters = (classes + 5) * 3
        '''
        # input shape : [batch, box_attr * anchor, lw, lh] ex) [1, 39, 19, 19]
        # 4dim [batch, box_attr * anchor, lh, lw] -> 5dim [batch, anchor, box_attr, lh, lw]
        # 5dim [batch, anchor, box_attr, lh, lw] -> 5dim [batch, anchor, lh, lw, box_attr] 
...
class Darknet53(nn.Module):
    ...
    def set_layer(self, layer_info):
        ...
        in_channels = [self.in_channels]    # First channel of input
